...
static void *my_alloc(void *ud, void *ptr, size_t sz) {
    ...
    if (ptr == NULL) {
        ...
        ++cookie->count;
        cookie->current += sz;
        if (cookie->current > cookie->max) { cookie->max = cookie->current; }
        // printf("%p + %lu\n",p,sz);
        return p;
    }
    --cookie->count;
...
static void init_obj(uint32_t    id,
                     float       x,
                     float       y,
                     float       z,
                     float       vx,
...
static void update_obj(struct aoi_space *aoi, uint32_t id) {
    int i;
    for (i = 0; i < 3; i++) {
...
static bool in_view(float pos1[3], float pos2[3]) {
    int i;
    for (i = 0; i < 3; i++) {
...
static void enterAOI(void *ud, uint32_t watcher, uint32_t marker) {
    printf(
        "op=enterAOI,watcher=[id=%d,pos=(%.1f,%.1f,%.1f)],marker=[id=%d,pos=(%.1f,%.1f,%.1f)]\n",
...
static void leaveAOI(void *ud, uint32_t watcher, uint32_t marker) {
    printf(
        "op=leaveAOI,watcher=[id=%d,pos=(%.1f,%.1f,%.1f)],marker=[id=%d,pos=(%.1f,%.1f,%.1f)]\n",
        watcher, OBJ[watcher].pos[0], OBJ[watcher].pos[1], OBJ[watcher].pos[2],
        marker, OBJ[marker].pos[0], OBJ[marker].pos[1], OBJ[marker].pos[2]);
    if (check_leave_aoi) {
...
static void test(struct aoi_space *aoi) {
    int i, j;
    ...
    for (i = 0; i < 7; i++) { aoi_enter(aoi, i, OBJ[i].pos, OBJ[i].mode); }
    for (i = 0; i < 100; i++) {
        if (i < 50) {
            for (j = 0; j < 7; j++) { update_obj(aoi, j); }
        } else if (i == 50) {
    ...
    if (ids != NULL && number != 0) {
        printf(
            "op=get_view_by_pos,pos=(%.1f,%.1f,%.1f),range=(%.1f,%.1f,%.1f),ids=",
    ...
    if (ids != NULL && number != 0) {
        printf(
            "op=get_view,id=%u,pos=(%.1f,%.1f,%.1f),range=(%.1f,%.1f,%.1f),ids=",
            id, OBJ[id].pos[0], OBJ[id].pos[1], OBJ[id].pos[2], range[0],
            range[1], range[2]);
        for (i = 0; i < number; i++) {
            if (i == number - 1) {
                printf("%u", (uint32_t)ids[i]);
    ...
    for (i = 0; i < 7; i++) { aoi_leave(aoi, i); }
}

int main() {
    struct alloc_cookie cookie = {0, 0, 0};

    struct aoi_space *aoi = aoi_create(my_alloc, &cookie, map_size, view_size,
                                       enterAOI, leaveAOI, NULL);
    test(aoi);
    aoi_release(aoi);
