...
static aoi_object *new_object(aoi_space *aoi, uint32_t id) {
...
static void delete_object(void *ud, aoi_object *obj) {
    aoi_space *aoi = ud;
...
static aoi_object *get_object(aoi_space *aoi, uint32_t id) {
    return map_get(aoi->objects, id);
}

static inline aoi_map_slot *mainposition(aoi_map *m, uint32_t id) {
...
map_insert(aoi_space *aoi, aoi_map *m, uint32_t id, aoi_object *obj) {
    ...
    if (mainposition(m, s->id) != s) {
        ...
        s->obj               = obj;
        s->next              = -1;
        if (temp_obj) { map_insert(aoi, m, temp_id, temp_obj); }
        return;
    } else if (s->obj == NULL) {
        s->id  = id;
        s->obj = obj;
...
static void rehash(aoi_space *aoi, aoi_map *m) {
    ...
    for (i = 0; i < old_size; i++) {
        ...
        if (s->obj) { map_insert(aoi, m, s->id, s->obj); }
    }
...
map_foreach(aoi_map *m, void (*func)(void *ud, aoi_object *obj), void *ud) {
    int i;
    for (i = 0; i < m->size; i++) {
        if (m->slot[i].obj) { func(ud, m->slot[i].obj); }
    }
}

static aoi_object *map_remove(aoi_map *m, uint32_t id) {
    aoi_map_slot *s = mainposition(m, id);
    for (;;) {
        if (s->id == id) {
...
static aoi_set *set_new(aoi_space *aoi) {
...
static void set_delete(aoi_space *aoi, aoi_set *set) {
...
static void set_add(aoi_space *aoi, aoi_set *set, void *elem) {
...
set_difference(aoi_space *aoi, aoi_set *set1, aoi_set *set2, aoi_set *result) {
    ...
    for (i = 0; i < number1; i++) {
        for (j = 0; j < number2; j++) {
            if (set1->slot[i] == set2->slot[j]) { break; }
        }
        if (j == number2) { set_add(aoi, result, set1->slot[i]); }
    }
}
...
inline static void copy_position(float des[3], float src[3]) {
    des[0] = src[0];
    des[1] = src[1];
    des[2] = src[2];
}

static void link_remove(aoi_space *aoi, char direction, aoi_object *node) {
    assert(aoi->origin != node);
    switch (direction) {
    case 'x':
        if (node->x_next) { node->x_next->x_prev = node->x_prev; }
        if (node->x_prev) { node->x_prev->x_next = node->x_next; }
        break;
    case 'y':
        if (node->y_next) { node->y_next->y_prev = node->y_prev; }
        if (node->y_prev) { node->y_prev->y_next = node->y_next; }
        break;
    case 'z':
        if (node->z_next) { node->z_next->z_prev = node->z_prev; }
        if (node->z_prev) { node->z_prev->z_next = node->z_next; }
        break;
    default: assert(false); break;
    }
}

static void link_insert(aoi_space * aoi,
                        char        direction,
                        aoi_object *after,
                        aoi_object *node) {
    switch (direction) {
    case 'x':
        ...
        node->x_prev = after;
        if (after->x_next != NULL) { after->x_next->x_prev = node; }
        after->x_next = node;
        break;
    case 'y':
        node->y_next = after->y_next;
        node->y_prev = after;
        if (after->y_next != NULL) { after->y_next->y_prev = node; }
        after->y_next = node;
        break;
    case 'z':
        node->z_next = after->z_next;
        node->z_prev = after;
        if (after->z_next != NULL) { after->z_next->z_prev = node; }
        after->z_next = node;
        break;
    default: assert(false); break;
    }
...
in_view(aoi_space *aoi, float pos1[3], float pos2[3], float view_size[3]) {
    int i;
    for (i = 0; i < 3; i++) {
        if (fabs(pos1[i] - pos2[i]) > view_size[i]) { return false; }
    }
    return true;
}
...
static void enterAOI(aoi_space *aoi, aoi_object *watcher, aoi_object *marker) {
    if (watcher->id == marker->id) { return; }
...
static void leaveAOI(aoi_space *aoi, aoi_object *watcher, aoi_object *marker) {
    if (watcher->id == marker->id) { return; }
    if (watcher->mode & MODE_WATCHER) {
...
aoi_space *aoi_create(aoi_Alloc         alloc,
                      void *            alloc_ud,
                      float             map_size[3],
                      float             view_size[3],
                      enterAOI_Callback cb_enterAOI,
...
static void *default_alloc(void *ud, void *ptr, size_t sz) {
    if (ptr == NULL) { return MALLOC(sz); }
    FREE(ptr);
    return NULL;
}

aoi_space *aoi_new(float             map_size[3],
                   float             view_size[3],
                   enterAOI_Callback cb_enterAOI,
                   leaveAOI_Callback cb_leaveAOI,
                   void *            cb_ud) {
    return aoi_create(default_alloc, NULL, map_size, view_size, cb_enterAOI,
                      cb_leaveAOI, cb_ud);
}
...
static bool change_mode(aoi_object *obj, const char *modestring) {
    int  i;
    bool change      = false;
...
void aoi_enter(aoi_space * aoi,
               uint32_t    id,
               float       pos[3],
               const char *modestring) {
    aoi_object *old_obj = get_object(aoi, id);
    if (old_obj != NULL) { aoi_leave(aoi, id); }
    int         i;
...
void aoi_move(aoi_space *aoi, uint32_t id, float pos[3]) {
    aoi_object *obj = get_object(aoi, id);
    if (obj == NULL) { return; }
    if (memcmp(obj->pos, pos, 3 * sizeof(float)) == 0) { return; }
    int         i;
    ...
    if (pos[0] < obj->pos[0]) {
        ...
        for (next = obj; next->x_next != NULL; next = next->x_next) {
            if (next->x_next->pos[0] >= pos[0]) { break; }
        }
        if (next != obj) {
            link_remove(aoi, 'x', obj);
    ...
    if (pos[1] < obj->pos[1]) {
        ...
        for (next = obj; next->y_next != NULL; next = next->y_next) {
            if (next->y_next->pos[1] >= pos[1]) { break; }
        }
        if (next != obj) {
            link_remove(aoi, 'y', obj);
    ...
    if (pos[2] < obj->pos[2]) {
        ...
        for (next = obj; next->z_next != NULL; next = next->z_next) {
            if (next->z_next->pos[2] >= pos[2]) { break; }
        }
        if (next != obj) {
            link_remove(aoi, 'z', obj);
