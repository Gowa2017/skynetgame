...
map_query(struct aoi_space *space, struct map *m, uint32_t id) {
    ...
    for (;;) {
        if (s->id == id) {
            if (s->obj == NULL) { s->obj = new_object(space, id); }
            return s->obj;
        }
        if (s->next < 0) { break; }
        s = &m->slot[s->next];
...
static void map_foreach(struct map *m,
                        void (*func)(void *ud, struct object *obj),
                        void *ud) {
    int i;
    for (i = 0; i < m->size; i++) {
...
static struct object *map_drop(struct map *m, uint32_t id) {
    uint32_t         hash = id & (m->size - 1);
...
static struct map *map_new(struct aoi_space *space) {
    int         i;
...
inline static void grab_object(struct object *obj) { ++obj->ref; }

static void delete_object(void *s, struct object *obj) {
    struct aoi_space *space = s;
    space->alloc(space->alloc_ud, obj, sizeof(*obj));
}

inline static void drop_object(struct aoi_space *space, struct object *obj) {
    --obj->ref;
    if (obj->ref <= 0) {
...
static struct object_set *set_new(struct aoi_space *space) {
    struct object_set *set = space->alloc(space->alloc_ud, NULL, sizeof(*set));
    set->cap               = PRE_ALLOC;
    set->number            = 0;
    set->slot =
        space->alloc(space->alloc_ud, NULL, set->cap * sizeof(struct object *));
    return set;
}
...
static void delete_pair_list(struct aoi_space *space) {
...
static void delete_set(struct aoi_space *space, struct object_set *set) {
    if (set->slot) {
        space->alloc(space->alloc_ud, set->slot,
                     sizeof(struct object *) * set->cap);
    }
...
static bool change_mode(struct object *obj, bool set_watcher, bool set_marker) {
    bool change = false;
    if (obj->mode == 0) {
        if (set_watcher) { obj->mode = MODE_WATCHER; }
        if (set_marker) { obj->mode |= MODE_MARKER; }
        return true;
    }
    if (set_watcher) {
...
static void set_push_back(struct aoi_space * space,
    ...
    if (set->number >= set->cap) {
        ...
        void *tmp = set->slot;
        set->slot =
            space->alloc(space->alloc_ud, NULL, cap * sizeof(struct object *));
...
static void set_push(void *s, struct object *obj) {
    struct aoi_space *space = s;
    int               mode  = obj->mode;
