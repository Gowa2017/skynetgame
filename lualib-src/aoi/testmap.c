...
static void init(int number, int start) {
    int i;
    for (i = 0; i < number; i++) { INDEX[i] = (i + start) * 8; }
}

static void mnew(struct map *m, uint32_t id) {
    //	printf("new %u\n",id);
...
static void mdelete(struct map *m, uint32_t id) {
...
static void test(struct map *m, int n, int start) {
    init(n, start);
    shuffle(n);
    int i;
    for (i = 0; i < n; i++) { mnew(m, INDEX[i]); }
    shuffle(n);
    n = rand() % (n / 2);
    for (i = 0; i < n; i++) { mdelete(m, INDEX[i]); }
}

static void check(void *ud, struct object *obj) {
    struct map *m = ud;
    printf("%u ", obj->id);
    mdelete(m, obj->id);
}
...
int main() {
    ...
    SPACE                      = aoi_create(my_alloc, 10.0, &cookie);

    struct map *m = map_new(SPACE);

    int i;
    for (i = 0; i < 10; i++) { test(m, 10, i * 10); }
    test(m, 100, 100);
    test(m, 200, 200);
    test(m, 500, 500);
