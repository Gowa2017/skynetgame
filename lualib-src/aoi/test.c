...
static void update_obj(struct aoi_space *space, uint32_t id) {
    int i;
    for (i = 0; i < 3; i++) {
...
static void message(void *ud, uint32_t watcher, uint32_t marker) {
    printf("%u (%f,%f) => %u (%f,%f)\n", watcher, OBJ[watcher].pos[0],
           OBJ[watcher].pos[1], marker, OBJ[marker].pos[0], OBJ[marker].pos[1]);
}
...
static void test(struct aoi_space *space) {
    ...
    for (i = 0; i < 100; i++) {
        if (i < 50) {
            for (j = 0; j < 4; j++) { update_obj(space, j); }
        } else if (i == 50) {
            aoi_update(space, 3, "d", OBJ[3].pos);
        } else {
            for (j = 0; j < 3; j++) { update_obj(space, j); }
        }
...
int main() {
    ...
    struct aoi_space *  space  = aoi_create(my_alloc, 10.0, &cookie);
