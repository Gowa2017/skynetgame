...
local toncat           = table.concat
local tunpack          = table.unpack

local enumtype         = { CHAR_TYPE_PLAYER  = 1, CHAR_TYPE_MONSTER = 2 }
...
local function updateviewmonster(monstertempid)
  ...
  for k, v in pairs(monsterview[monstertempid]) do
    ...
    otherobj = { tempid = othertempid, agent  = OBJ[k].agent }
    ...
    elseif distance > AOI_RADIS2 and distance <= LEAVE_AOI_RADIS2 then
      ...
      if v then table.insert(leavelist, otherobj) end
  ...
  -- 视野范围内移动
  for _, v in pairs(movelist) do skynet.send(v, "lua", "updateaoiobj", myobj) end

  skynet.send(myobj.agent, "lua", "updateaoilist", myobj.tempid, enterlist,
              leavelist)
end
...
local function updateviewplayer(viewertempid)
  ...
  -- 离开他人视野
  local leavelist   = { playerlist  = {}, monsterlist = {} }
  -- 进入他人视野
  local enterlist   = { playerlist  = {}, monsterlist = {} }
  -- 通知他人自己移动
  local movelist    = { playerlist  = {}, monsterlist = {} }
  ...
  for k, v in pairs(playerview[viewertempid]) do
    ...
    elseif distance > AOI_RADIS2 and distance <= LEAVE_AOI_RADIS2 then
      ...
      if v then inserttotablebytype(leavelist, otherobj, othertype) end
  ...
  if not table.empty(leavelist.monsterlist) or
    ...
    skynet.send(mapagent, "lua", "updateaoiinfo", monsterenterlist,
                monsterleavelist, monstermovelist)
  end
...
--- aoi回调，由 CAOI 发送  text aoicallback 消息过来
--- 一个对象，可能会有多个消息返回
function CMD.aoicallback(w, m)
  assert(OBJ[w], w)
  assert(OBJ[m], m)

  if playerview[OBJ[w].tempid] == nil then playerview[OBJ[w].tempid] = {} end
  playerview[OBJ[w].tempid][OBJ[m].tempid] = true

  -- 怪物视野内的玩家
  if OBJ[m].type ~= enumtype.CHAR_TYPE_PLAYER then
    if monsterview[OBJ[m].tempid] == nil then monsterview[OBJ[m].tempid] = {} end
...
-- 添加到 aoi，但新进入的对象，视图的更新会推迟到 AOI 的回调
function CMD.characterenter(obj)
  ...
  assert(pcall(skynet.send, aoi, "text", sfmt(FMT, "update", obj.tempid,
                                              obj.movement.mode,
                                              tunpack(obj.movement.pos))))
  need_update = true
end
...
function CMD.characterleave(obj)
  ...
  assert(pcall(skynet.send, aoi, "text",
               sfmt(FMT, "update", obj.tempid, "d", tunpack(obj.movement.pos))))
  OBJ[obj.tempid] = nil
  if playerview[obj.tempid] then
    local monsterleavelist = { tempid      = obj.tempid, monsterlist = {} }
    ...
    if not table.empty(monsterleavelist.monsterlist) then
      skynet.send(mapagent, "lua", "updateaoiinfo", { monsterlist = {} },
