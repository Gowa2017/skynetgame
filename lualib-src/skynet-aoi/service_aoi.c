...
static void *my_alloc(void *ud, void *ptr, size_t sz) {
    ...
    if (ptr == NULL) {
        ...
        ++cookie->count;
        cookie->current += sz;
        if (cookie->max < cookie->current) { cookie->max = cookie->current; }
        return p;
    }
    --cookie->count;
    cookie->current -= sz;
...
static void callbackmessage(void *ud, uint32_t watcher, uint32_t marker) {
    ...
    size_t sz = getnumbercount(watcher) + getnumbercount(marker) +
                strlen("aoicallback") + 2;
    char *msg = skynet_malloc(sz);
    memset(msg, 0, sz);
    sprintf(msg, "aoicallback %d %d", watcher, marker);
    // caoi server的启动在laoi启动之后，handle理论是caoi = laoi + 1
    //如果失败,就需要换方式了
    skynet_send(ctx, 0, skynet_current_handle() - 1,
                PTYPE_TEXT | PTYPE_TAG_DONTCOPY, 0, (void *)msg, sz);
}
...
static void _parm(char *msg, int sz, int command_sz) {
    ...
    int i;
    for (i = command_sz; i < sz; i++) { msg[i - command_sz] = msg[i]; }
    msg[i - command_sz] = '\0';
}

static void _ctrl(struct skynet_context *ctx,
                  struct aoi_space      *space,
                  const void            *msg,
                  int                    sz) {
    char tmp[sz + 1];
...
struct aoi_space_plus *caoi_create(void) {
    struct aoi_space_plus *space_plus =
        skynet_malloc(sizeof(struct aoi_space_plus));
...
static int caoi_cb(struct skynet_context *context,
                   void                  *ud,
                   int                    type,
                   int                    session,
                   uint32_t               source,
    ...
    switch (type) {
    case PTYPE_TEXT: _ctrl(context, space, msg, (int)sz); break;
    }
